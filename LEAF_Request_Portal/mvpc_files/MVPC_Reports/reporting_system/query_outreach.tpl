...
function getData(dateStart, dateEnd) {
    ...
    query.importQuery({"terms":[{"id":"deleted","operator":"=","match":0,"gate":"AND"}],"joins":["service","initiatorName"],"sort":{},"getData":["11","13","2","9","14","10","6","1","8","12","17","4","5","16","15"]});
   	query.addTerm('categoryID', '=', 'form_ce8a4');
 	let admin = document.getElementById('administration-select').value;
    console.log(admin);
    if(admin === 'NCA' || admin === 'VBA' || admin === 'VHA'){
    	query.addDataTerm('data', 11, '=', admin);
    }
    ...
    if (dateStart !== undefined && dateStart !== ""){  
        ...
        //console.log(new Date(dsAdjusted));
...
$(function() {
    ...
    $('#headerTab').html('Outreach Search');
...
<p>Select from the date pickers to search within a date range (date of row entry), or from the admin dropdown to search within a specific administration.</p>
<p>Only the selected values will be used (For example, select no dates and no administration to search all records)</p>
