<!-- use form with orgchart emp format and list of emails to get output 
for employee information.  modify for backup assignment.-->
    ...
    function makeRequests(categoryID, requestData) {
        return new Promise(function(resolve, reject){
            ...
            if (failedRequests.length === (sheet_data.cells.length - 1)) {
                requestStatus.html('See log for details.');
                requestStatus.append(
                    '<br/><br/>' +
                    'Results: <br/>' + failedRequests.join("<br/>"));
                ...
                function answerQuestions() {
                    ...
                    if (completed === indicatorArray.length) {
                        ...
                        if (indicatorColumn === "-1") {
                            ...
                            switch (currentFormat) {  //keeping format so that the content is more obvious 
                                case 'orgchart_employee':
                                    if (indicatorColumn === "A"){
                                        let sheetEmpUID_A = typeof (row["A"]) !== "undefined" && row["A"] !== null ? row["A"].toString().trim() : '';
                                        let sheetEmpUID_B = typeof (row["B"]) !== "undefined" && row["B"] !== null ? row["B"].toString().trim() : '';
                                        if (!isNaN(sheetEmpUID_A) && !isNaN(sheetEmpUID_B) && sheetEmpUID_A !== '' && sheetEmpUID_B !== ''){
                                            console.log('primary: ', sheetEmpUID_A, 'backup: ', sheetEmpUID_B);
                                            //ADD BACKUPS
                                            
                                            $.ajax({
                                                type: 'POST',
                                                url: '<!--{$orgchartPath}-->/api/employee/' + sheetEmpUID_A + '/backup',
                                                data: {backupEmpUID: sheetEmpUID_B,
                                                    CSRFToken: '<!--{$CSRFToken}-->'},
                                                success: function(response) {
                                                    console.log('S', response);
                                                    //don't want to actually make requests, just want the info after
                                                    requestData['failed'] = indicatorColumn + (titleIndex + 1) + 'backup assignment, primaryID, '+ sheetEmpUID_A + ', backupID,' + sheetEmpUID_B;
                                                    completed++;
                                                    answerQuestions().then(function(){
                                                        resolve();
                                                    })
                                                ...
                                                fail: function(response) {
                                                    ...
                                                    //don't want to actually make requests, just want the info after
                                                    requestData['failed'] = indicatorColumn + (titleIndex + 1) + 'error assigning backup for pair: ' + sheetEmpUID_A + ' ' + sheetEmpUID_B;
                                                    completed++;
                                                    answerQuestions().then(function(){
                                                        resolve();
                                                    })
                                            ...
                                            requestData['failed'] = indicatorColumn + (titleIndex + 1) + ", empUID format error (blank or not numeric)."; 
                                            completed++;
                                            answerQuestions().then(function(){
                                                resolve();
                                            })
                                        ...
                                        //don't do anything with other columns, B is used above to get backup for "A", other cols additional info (eg email) for easier tracking
                                        requestData['failed'] = indicatorColumn + (titleIndex + 1) + ', skipping secondary column';
                                        completed++;
                                        answerQuestions().then(function(){
                                            resolve();
                                        })
