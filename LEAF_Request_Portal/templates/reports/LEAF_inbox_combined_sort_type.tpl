...
<script>
     ...
     *                    Available columns include: 'UID,service,title,status,days_since_last_action'
    ...
    let sites = [
        {
            ...
            nonadmin: false,
            columns: {
                'form_1e8cf': 'UID,service,2511,days_since_last_action',
                'form_9db12': 'UID,2302',
            }
        },
        {
	...
	let headerDefinitions = {
        ...
        'title': function(site) {
            ...
            name: 'Title', indicatorID: 'title', editable: false, callback: function (data, blob) {
                ...
                $('#' + data.cellContainerID).html('<a href="'+ site.url +'index.php?a=printview&recordID=' + data.recordID + '" target="_blank">'
                    ...
                    + '<div id="inboxForm' + hash + '_' + data.recordID + '" style="background-color: white; display: none; height: 300px; overflow: scroll"></div>');
                    ...
                    if ($('#inboxForm' + hash + '_' + data.recordID).html() == '') {
                        ...
                        $.ajax({
                            ...
                            success: function (res) {
                                $('#inboxForm' + hash + '_' + data.recordID).html(res);
                                $('#inboxForm' + hash + '_' + data.recordID).slideDown();
        ...
        'days_since_last_action': function(site) {
            return {
            name: 'Days since last action', indicatorID: 'daysSinceLastAction', editable: false, callback: function (data, blob) {
                    let daysSinceAction;
        ...
        'email_reminder': function(site) {
            return {
            name: 'Email Reminder', indicatorID: 'emailReminder', editable: false, callback: function (data, blob) {
                    let daysSinceAction;
                    let recordBlob = blob[data.recordID];
                    if(recordBlob.action_history != undefined) {
                        // Get Last Action no matter what (could change for non-comment)
    ...
    function renderInbox() {
        for (let i in sites) {
            ...
            sortedDepDesc.forEach(categoryName => {
                ...
                buildDepInbox(dataInboxes[sites[i].url], categoryIDs[categoryName], categoryName, recordIDs, sites[i]);
            });
        }
        $('#loading').slideUp();
        $('.inbox').fadeIn();
    ...
    function buildDepInbox(res, categoryIDs, categoryName, recordIDs, site) {
        ...
        $('#siteContainer' + hash).append('<div id="depContainer' + hash + '_' + depID + '" style="border: 1px solid black; background-color: #e6e4b9; cursor: pointer; margin: 4px">'
            + '<div id="depLabel' + hash + '_' + depID + '" class="depInbox" style="padding: 8px"><span style="float: right; text-decoration: underline; font-weight: bold">View '+ recordIDs.length +' requests</span>'
                    ...
                    site.columns[categoryID].split(',').forEach(col => {
                        ...
                        if(isNaN(parseInt(col))
                          && headerDefinitions[col] != undefined) {
                            ...
                            customCols.push({name: label, indicatorID: parseInt(col)});
                        }
                    });
    ...
    function loadInboxData(site) {
        ...
        query.setExtraParams('&x-filterData=recordID,categoryIDs,categoryNames,title,service,submitted,deleted,stepID,blockingStepID,lastStatus,stepTitle,action_history.time' + nonAdminParam);
