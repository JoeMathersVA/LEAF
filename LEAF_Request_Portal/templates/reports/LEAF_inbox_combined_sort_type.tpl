...
<script>
    ...
    function renderInbox() {
        for (let i in sites) {
            ...
            sortedDepDesc.forEach(categoryName => {
                ...
                buildDepInbox(dataInboxes[sites[i].url], categoryName, recordIDs, sites[i]);
            });
        }
        $('#loading').slideUp();
        $('.inbox').fadeIn();
    ...
    function buildDepInbox(res, categoryName, recordIDs, site) {
    ...
    function loadInboxData(site, nonadmin) {
        ...
        query.setExtraParams('&x-filterData=recordID,categoryNames,title,submitted,deleted,stepID,blockingStepID,lastStatus,stepTitle' + nonAdminParam);
        query.importQuery({"terms":[{"id":"stepID","operator":"!=","match":"resolved","gate":"AND"},{"id":"deleted","operator":"=","match":0,"gate":"AND"}],"joins":["service","categoryName","status"],"sort":{}});
